...
(function (global) {
  ...
  /* ---------- CSS (scoped: .tk-) ---------- */
  var CSS = [
    /* Faz 5 — .tk-navy2 BİLEREK Faz 1 tokenına bağlanmadı: sadece opak, kendi içinde
       kapalı kontrast taşıyan rozet arkaplanları (stat çipi, plan paneli, alt video butonu)
       için kullanılıyor, hepsi sabit beyaz metinle eşleşiyor — kart teması değişse de
       bu rozetlerin okunurluğu etkilenmiyor, temayı takip etmelerine gerek yok. */
    '.tk-root{--tk-navy:var(--text-primary,#0B2545);--tk-navy2:#143A5E;--tk-soft:var(--surface-2,#EAF0F6);--tk-orange:var(--accent-orange,#F26522);--tk-osoft:var(--accent-soft,#FDEBE0);--tk-line:var(--surface-border,#D8DEE6);--tk-ink:var(--text-primary,#1B2430);--tk-muted:var(--text-secondary,#607285);color:var(--tk-ink);font-family:inherit;line-height:1.55;}',
    '.tk-root *{box-sizing:border-box;}',
    '.tk-intro{background:var(--surface-1,#fff);border:1px solid var(--tk-line);border-radius:14px;padding:16px 20px;margin-bottom:18px;}',
    '.tk-intro p{font-size:14.5px;color:var(--text-secondary,#39485a);margin:0;}',
    ...
    '.tk-mat{background:var(--surface-1,#fff);border:1px solid var(--tk-line);border-radius:12px;padding:14px 16px;}',
    '.tk-mat h4{font-weight:700;font-size:15.5px;color:var(--tk-navy);margin:0 0 3px;text-transform:uppercase;letter-spacing:.02em;}',
    '.tk-mat p{font-size:13px;color:var(--text-secondary,#45566a);margin:0;}',
    ...
    '.tk-drill{background:var(--surface-1,#fff);border:1px solid var(--tk-line);border-radius:16px;margin-bottom:16px;overflow:hidden;}',
    ...
    '.tk-amac{font-size:15px;color:var(--text-primary,#2c3a4a);margin:0 0 12px;}',
    '.tk-amac b{color:var(--tk-navy);}',
    '.tk-steps{list-style:none;margin:0 0 12px;padding:0;display:flex;flex-direction:column;gap:6px;}',
    '.tk-steps li{position:relative;padding-left:22px;font-size:14px;color:var(--text-primary,#39485a);}',
    ...
    '.tk-stat{background:var(--tk-navy2);color:#fff;border-radius:9px;padding:6px 11px;line-height:1.1;}',
    ...
    '.tk-vid.tk-alt{background:var(--tk-navy2);}',
    '.tk-vid.tk-alt:hover{background:#0B2545;}',
    '.tk-vid svg{width:14px;height:14px;}',
    '.tk-novid{font-weight:600;font-size:12.5px;color:var(--tk-muted);}',
    '.tk-plan{background:var(--tk-navy2);border-radius:16px;padding:22px 24px;color:#fff;}',
